<template>
  <div>
    <form action method="get" name="frmForm" id="_frmForm" @submit.prevent="updateEmp">
      <table class="content_table">
        ...
        <tr>
          <td colspan="2" style="height: 50px; text-align: center;">
            <button type="submit" name="button">사원 수정</button>
          </td>
        </tr>
      </table>
    </form>
...
export default {
  ...
  methods: {
    updateEmp() {
      ...
      axios
        .put("http://localhost:9999/api/updateEmployee",{
          id : this.id,
          name : this.emp.cname,
          mailid : this.emp.cmailid,
          start_date : this.emp.cstart_date,
          manager_id : this.emp.cmanager_id,
          title : this.emp.ctitle,
          dept_id : this.emp.cdept_id,
  ...
  mounted() {
    ...
    axios
      ...
      .then(response => {
        this.remp = response.data;
        this.emp.cname = this.remp.name;
        this.emp.cmailid = this.remp.mailid;
        this.emp.cstart_date = this.remp.start_date;
        this.emp.cmanager_id = this.remp.manager_id;
        (this.emp.ctitle = this.remp.title),
          (this.emp.cdept_id = this.remp.dept_id),
          (this.emp.csalary = this.remp.salary),
          (this.emp.ccommission = this.remp.commission);
      })
      .catch(() => {
        this.errored = true;
      })
